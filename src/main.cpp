...
unsigned int nTargetSpacing     = 60; // 1 minute block spacing (Old Block Spacing)   
unsigned int nStakeTargetSpacing = 60; // 1-minute block spacing (New Block Spacing)
unsigned int nStakeMinAge       = 1 * 60 * 60; // 1 hours (Original Minimum Stake Age)
unsigned int nStakeMinAgeV2 = 1 * 60 * 60; // 1 Hour Minimum Stake Age (New Minimum Stake Age)
unsigned int nStakeMaxAge       = -1; // Unlimited (Original Minimum Stake Age)
...
const string strMessageMagic = "Camorracoin Signed Message:\n";
...
{
    int64_t nRewardCoinYear, nSubsidyLimit = 100 * COIN;

   // bool fPrintCreation = GetBoolArg("-printcreation");

    CBigNum bnRewardCoinYearLimit = COIN_YEAR_REWARDV2; // Base stake mint rate, 3% year interest
    CBigNum bnTarget;
...
// camorracoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Camorracoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        */
        ...
        const char* pszTimestamp = "KumorrahCuhoin";
        CTransaction txNew;
        txNew.nTime = 1420781321;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x0669d90ca4625bdba2d85e09d38c081b938eb21ea7ce6ee3f30487b27a06bf63"));
        block.print();
